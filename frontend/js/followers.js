...
function cargarSeguidores(page){
    ...
    conexionSeguidores.open('GET',"http://127.0.0.1/backend/followers.php?api_key="+getCookie("api_key")+"&id="+findGetParameter("id")+"&page="+page);
        ...
        if(conexionSeguidores.readyState == 4 && conexionSeguidores.status == 200) {
            if(conexionSeguidores.responseText.length>0){
                ...
                for(let i=0;i<data.length;i++){
                    ...
                    if(imageExists("/backend/img/"+data[i].id+"_avatar.jpg")){
                        avatar_img="/backend/img/"+data[i].id+"_avatar.jpg";
                    }

                    let tweet = `
                    <div class="tweet" id="`+data[i].id+`">
                        <div class="foto-perfil" onclick="window.location.href = 'profile.html?id=`+data[i].id+`'">
                            <img src="`+avatar_img+`">
                        </div>

                        <div class="contenido-tweet" style="padding-top: 1.5%;">
                            <span style="font-size:17px;" onclick="window.location.href = 'profile.html?id=`+data[i].id+`'"><b>`+data[i].name_twitter+`</b> @`+data[i].nick+`</span>
                            <a id="boton" onclick="window.location.href = 'profile.html?id=`+data[i].id+`'" style="cursor: pointer;width: 15%;float: right;margin-top:-15px;font-size: 15px;" class="boton-secundario seguir">Ver perfil</a>
                        </div>
                    </div>
                    `;
                ...
                let lista = document.getElementById("lista-seguidores");
                if(lista.innerHTML==""){
                    lista.innerHTML+="<center style='margin-top:20px;'>Este usuario no tiene seguidores</center>";
                }
            }
        }
    }
