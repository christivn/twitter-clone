    ...
    if(conexion.readyState == 4 && conexion.status == 200) {
        ...
        if(data.code=="200"){
            ...
            function cargarTweets(page){
                conexion_profile_feed=new XMLHttpRequest();
                conexion_profile_feed.addEventListener('readystatechange',callBackProfileFeed);
                conexion_profile_feed.open('GET',"http://127.0.0.1/backend/profileFeed.php?api_key="+getCookie("api_key")+"&id="+id+"&page="+page);
                conexion_profile_feed.send();

                function callBackProfileFeed() {
                    if(conexion_profile_feed.readyState == 4 && conexion_profile_feed.status == 200) {
                        if(conexion_profile_feed.responseText.length>0){
                            let data=JSON.parse(conexion_profile_feed.responseText);

                            let nick = document.getElementById("arroba").textContent;
                            let name_twitter =document.getElementById("nombre-perfil").textContent;
                            ...
                            for(let i=0;i<data.length;i++){
                                ...
                                if(data[i].img_url.length!=0){
                                    img=`<div id="`+data[i].tweet_id+`_img" style="background-color: #cccccc;width:90%;height:450px;margin-bottom:1rem;border-radius:5px;background-repeat: no-repeat;background-size: cover;"></div>`;
                                }
                                ...
                                <div class="tweet" id="`+data[i].tweet_id+`">
                                    ...
                                    <div class="contenido-tweet">
                                        <span><b>`+name_twitter+`</b> `+nick+` ·  `+fecha_tweet+ajustes+`</span>
...
function mostrarModal(tweet_id){
    let modalEliminar = document.getElementById("ajustes-modal");
    let botonEliminar = document.getElementById("eliminarTweet");
    ...
    botonEliminar.onclick = function() {
        conexionEliminar=new XMLHttpRequest();
        conexionEliminar.addEventListener('readystatechange',callBackEliminar);
        conexionEliminar.open('GET',"http://127.0.0.1/backend/deleteTweet.php?api_key="+getCookie("api_key")+"&tweet_id="+tweet_id.id);
        conexionEliminar.send();

        function callBackEliminar() {
            if(conexionEliminar.readyState == 4 && conexionEliminar.status == 200) {
                location.reload();
            }
        }
    }
